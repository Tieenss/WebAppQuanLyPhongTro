...
export default function LandlordIssuesPage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case "DANG_XU_LY":
                return <span
                    className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs font-medium">Đang xử lý</span>;
            case "HOAN_THANH":
                return <span
                    className="px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-medium">Hoàn thành</span>;
            default:
                return <span
                    className="px-2 py-1 bg-slate-100 text-slate-700 rounded text-xs font-medium">{status}</span>;
        }
    };
    ...
    return (
        <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-start sm:items-center gap-3">
                    <Link href="/landlord/management"
                          className="text-slate-400 hover:text-blue-600 transition-colors mt-1 sm:mt-0">
                        <ArrowLeft className="w-6 h-6"/>
                    </Link>
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Quản lý sự
                            cố</h1>
                        <p className="mt-1 text-sm text-slate-500">Theo dõi và cập nhật trạng thái các yêu cầu sửa chữa,
                            sự cố từ người thuê.</p>
                    </div>
                </div>
                <button
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div
                    className="p-4 sm:p-5 border-b border-slate-100 flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-50/50">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        ...
                        <tr>
                            <th scope="col"
                                className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Phòng
                                / Người báo
                            </th>
                            <th scope="col"
                                className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Chi
                                tiết sự cố
                            </th>
                            <th scope="col"
                                className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Mức
                                độ
                            </th>
                            <th scope="col"
                                className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Trạng
                                thái
                            </th>
                            <th scope="col"
                                className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Ngày
                                báo cáo
                            </th>
                            <th scope="col"
                                className="px-6 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">Hành
                                động
                            </th>
                        </tr>
                        </thead>
                        ...
                        {isLoading ? (
                            <tr>
                                <td colSpan={6} className="px-6 py-10 text-center text-slate-500">Đang tải dữ liệu...
                                </td>
                            </tr>
                        ) : filteredIssues.length === 0 ? (
                            <tr>
                                <td colSpan={6} className="px-6 py-10 text-center text-slate-500">Không tìm thấy sự cố
                                    nào.
                                </td>
                            </tr>
                        ) : (
                            filteredIssues.map((issue: Issue) => (
                                <tr key={issue.id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div
                                            className="text-sm text-slate-500">{issue.tenantName || 'Không xác định'}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="font-medium text-slate-800">{issue.issueType}</div>
                                        <div
                                            className="text-sm text-slate-500 truncate max-w-xs">{issue.description}</div>
                                        {issue.imageUrl && (
                ...
                <div
                    ...
                    <div
                        ...
                        <div
                            ...
                            <h3 className="text-lg font-bold text-slate-900">Cập nhật sự cố
                                phòng {selectedIssue.roomNumber}</h3>
                        ...
                        <form onSubmit={handleUpdateStatus} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <div
                                    className="p-3 bg-slate-50 rounded-lg text-sm text-slate-700 border border-slate-200 h-24 overflow-y-auto">
                            ...
                            {selectedIssue.imageUrl && (
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Ảnh đính
                                        kèm</label>
                                    <img src={selectedIssue.imageUrl} alt="Sự cố"
